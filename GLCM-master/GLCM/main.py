...
def Glcm(image_path, output_path):
    nbit = 64  # gray levels
    ...
    img = np.array(img.convert('L'))  # If the image has multi-bands, it needs to be converted to grayscale image
    ...
    result = plt.savefig(output_path
                         , format='png'
                         , bbox_inches='tight'
                         , pad_inches=0
                         , dpi=300)
...
if __name__ == '__main__':
    ...
    for name in os.listdir(input_path):
        ...
        result_path = os.path.join(output_path,name+"_1.png")
        Glcm(image_path,result_path)
